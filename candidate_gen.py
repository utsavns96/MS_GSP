        ...
        for s2 in Fk_1:
            ...
            if (not s1_first_smallest):
                for index, i in enumerate(s2[:-1]):
                    if index < len(s2[:-1]) -1 and mis[i]<s2_last_mis:
                        s2_last_smallest = False
                    elif index == len(s2[:-1]) -1 and mis[i]<=s2_last_mis:
            ...
            # condtition 3: default case
                ...
                print("default case")
                join_step(Fk_1) # need to update this to take s1 and s2 only instead of Fk_1 - iteration of sequences happens here
